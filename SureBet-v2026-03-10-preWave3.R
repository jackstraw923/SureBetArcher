...
# ── Delta / Hedge signals ──────────────────────────────────────────────────────
# FIXED: Bug #12 — HEDGE_TRIGGER_CLV was defined here AND again ~65 lines below
# alongside CLOSE_TRIGGER_CLV. The second definition silently won; if values ever
# diverged during tuning, check_position_actions() would use the wrong threshold.
# Canonical definition kept below with CLOSE_TRIGGER_CLV and CLOSE_MIN_PROFIT.
# (See "── Hedge / Close trigger thresholds" block)
...
# ── MLB-specific ───────────────────────────────────────────────────────────────
# FIXED: Bug #13 — MLB_SPORT_KEY / MLB_SEASON / BSR params were redefined
# identically mid-script inside the MLB pipeline section. Duplicate block
# deleted. This block is now the ONLY place to make the March 27 flip.
# ↓↓ MARCH 27: change both values below, nowhere else ↓↓
MLB_SPORT_KEY  <- "baseball_mlb_preseason"  # → "baseball_mlb" on March 27
MLB_SEASON     <- 2025                       # → 2026 on March 27
  ...
  log_df %>%
    mutate(
      status = case_when(
        ...
        # FIXED: Bug #11 — NA game_start (MLB preseason, some soccer rows) made
        # `now >= game_start` evaluate to NA, which case_when treats as non-matching.
        # Positions with NA game_start were never auto-expired or advanced to LIVE.
        # Guard added: only attempt clock-driven transitions when game_start is known.
        status == "IDENTIFIED" & !is.na(game_start) & now >= game_start ~ "EXPIRED",
        # OPEN → LIVE the moment the game starts
        status == "OPEN"       & !is.na(game_start) & now >= game_start ~ "LIVE",
        # Everything else — no automatic advance beyond LIVE
        # (HEDGED, CLOSED, SETTLED require human confirmation or settler block)
        # NOTE: positions with NA game_start remain IDENTIFIED/OPEN until game_start
        # is populated (e.g., after March 27 MLB season-start flip).
        TRUE ~ status
      )
    )
}
  ...
  log_df %>%
    mutate(
      pnl = case_when(
        ...
        # FIXED: Bug #1 — result is character "W"/"L", not numeric 1/0;
        # result == 1 always evaluated FALSE, making every winning settled
        # bet return -stake instead of the correct profit.
        status == "SETTLED" & is.na(hedge_stake) ~
          if_else(result == "W",
...
write_bet_log <- function(df, log_dir = "logs") {
  ...
  if (file.exists(log_path)) {
    ...
    # FIXED: Error 2 — pre-lifecycle log files have no position_id column;
    # filter() threw "uninitialised column" warning. Guard adds it before use.
    if (!"position_id" %in% names(existing)) {
      existing$position_id <- NA_character_
    }
    # FIXED: Error 1 — empty tibble result col defaults to <dbl> NA; settled
    # logs have <chr>. Coerce new_rows before bind to guarantee type match.
    new_rows  <- new_rows %>% mutate(result = as.character(result))
...
write_spread_total_log <- function(df, log_dir = "logs") {
  ...
  if (file.exists(log_path)) {
    ...
    # FIXED: Error 1 — coerce result to character before bind_rows().
    new_rows  <- new_rows %>% mutate(result = as.character(result))
    new_only  <- new_rows %>% filter(!position_id %in% existing$position_id)
    if (nrow(new_only) > 0) {
      write_csv(bind_rows(existing, new_only), log_path)
...
# ============================================================
# CORE VALUE BET FUNCTION
# ============================================================
# FIXED: Bug #4 — calc_value_bets was defined AFTER the MLB pipeline block
# which calls it, causing "could not find function" on a clean run.
# Moved here so it is available to all sport modules (NBA, NCAAB, NHL, MLB).
# Function body is unchanged.

#' Calculate value metrics for one sport.
#' @param games_df       Enriched games df with home_ml, away_ml, and pyth cols.
#' @param pyth_home_col  Unquoted column name for home Pythagorean win probability.
#' @param pyth_away_col  Unquoted column name for away Pythagorean win probability.
#' @param sport_label    String label written to the sport column ("NBA", "NHL", etc.)
calc_value_bets <- function(games_df, pyth_home_col, pyth_away_col, sport_label) {
  ...
  games_df %>%
    ...
    mutate(
      ...
      # Step 5: Full Kelly → Quarter Kelly
      # FIXED: Bug #3 — if home_ml or away_ml == 1.0 (API artifact), denominator
      # is zero → Inf Kelly → corrupts all_raw_kelly sum and scale factor.
      # pmax(..., 1e-6) clamps denominator; any odds that bad produce ~0 Kelly anyway.
      home_kelly_full = pmax((home_ml - 1) * !!pyth_home_col - (1 - !!pyth_home_col),
                             0) / pmax(home_ml - 1, 1e-6),
      away_kelly_full = pmax((away_ml - 1) * !!pyth_away_col - (1 - !!pyth_away_col),
                             0) / pmax(away_ml - 1, 1e-6),
...
# ============================================================
# FIXED: Bug (ordering) — calc_expected_spread and calc_expected_total were
# defined at ~line 1970, well after the NBA enrichment block that calls them.
# Moved here alongside calc_value_bets so all three are defined before any
# sport module runs. Function bodies unchanged.
...
# SETTLER FUNCTIONS
# ============================================================
# FIXED: Bug (ordering) — settle_mlb_log, settle_paper_day, and
# settle_soccer_log were all defined after the settler execution blocks
# that call them, causing "could not find function" errors on a clean run.
# Moved here alongside the other pre-NBA function definitions.
...
settle_paper_day <- function(ml_log_path, bankroll_at_open, st_log_path = NULL,
  ...
  if (!is.null(soccer_pnl_by_date_arg) && nrow(soccer_pnl_by_date_arg) > 0) {
    ...
    else as.Date(ml_log$log_date[1])
  ...
  bind_rows(
    ...
    valued %>% filter(draw_value) %>%
      transmute(
        ...
        home_value, away_value = FALSE,
        bet_ev = draw_ev,
        # FIXED: Bug #7 — was `raw_kelly = home_kelly_q`, which sized every draw
        # bet using the HOME team's Kelly fraction regardless of the draw's own
        # edge/odds. Corrected to draw_kelly_q.
        raw_kelly = draw_kelly_q,
        # ── Lifecycle fields ──────────────────────────────
        position_id   = make_position_id(sport, game_date,
                                         home_team_norm, away_team_norm,
...
# FIXED: Bug (new) — TOA preseason odds often omit totals columns entirely.
# Guard against missing over_outcomes_point before calling calc_mlb_totals()
# so that a missing totals market never kills the MLB ML pipeline.
# FIXED: MLB preseason — TOA returns no totals market at all; confirmed columns
# over_outcomes_point / over_outcomes_price / under_outcomes_price are absent
# from mlb_games. Guard checks all three before calling calc_mlb_totals() so
# the ML pipeline continues normally when totals are unavailable.
MLB_TOTALS_COLS <- c("over_outcomes_point", "over_outcomes_price", "under_outcomes_price")

if (!all(MLB_TOTALS_COLS %in% names(mlb_games))) {
  cat("⚠️  MLB totals unavailable (preseason) — skipping calc_mlb_totals()\n")
  totals_mlb <- tibble()
} else {
...
# ── Dynamic LEAGUE_AVG overrides — single source of truth ────────────────────
# FIXED: Bug #5 — these were computed again ~110 lines below inside the unified
# value bet section, creating a silent double-assignment. Second block deleted.
LEAGUE_AVG_NBA <- mean(
...
# FIXED: Bug #10 — nrow(nrow(...)) always returns 1 because nrow() returns a
# scalar integer and nrow() of a length-1 vector is 1. Outer nrow() removed.
