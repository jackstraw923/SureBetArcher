...
name_crosswalk <- tribble(
  ...
  "San José St Spartans",             "San Jose State Spartans"
)
...
tennis_best_odds <- tennis_h2h %>%
  ...
  select(game_date, sport_key, home_team, away_team,
...
tennis_picks <- if (file.exists(challenger_xlsx)) {
  ...
  raw_matches <- read_xlsx(challenger_xlsx, sheet = "Matches",
                           col_names = FALSE,   # use positional — header row is unusable
                           skip = 1)            # skip the header row entirely

  # ── RAW XLSX DIAGNOSTIC — remove after column mapping is confirmed ────────
  cat(sprintf("\n📋 Matches tab: %d rows × %d columns\n",
              nrow(raw_matches), ncol(raw_matches)))
  cat("First 3 rows (raw, no column names applied yet):\n")
  print(raw_matches[1:min(6, nrow(raw_matches)), 1:min(26, ncol(raw_matches))],
        n = 6, width = 200)
  cat(sprintf("Column count: %d  (expected 24 — A through X)\n", ncol(raw_matches)))
  # Show which columns have data (non-NA) to identify layout
  col_fill <- sapply(raw_matches, function(x) sum(!is.na(x)))
  cat("Non-NA counts per column:\n")
  print(data.frame(col = paste0("...", seq_along(col_fill)), n_filled = col_fill),
        row.names = FALSE)
  cat("──────────────────────────────────────────────────────────────────\n\n")

  # Name columns positionally (A=col1 ... X=col24)
  # Column mapping confirmed from raw diagnostic (2026-03-12):
  #   col1  = player name (with seeding/nationality)
  #   col2  = decimal odds
  #   col3  = prob_c (current round survival)
  #   col4  = prob_d (next round survival)
  #   col5  = prob_e (round+2 survival)
  #   col6  = prob_f (round+3 / win-tournament)
  #   col7–11 = empty (all NA)
  #   col12–13 = intermediate model probs (not used)
  #   col14 = model_prob_v_alt (spreadsheet EV%, 81 filled)
  #   col15 = implied_alt (62 filled)
  #   col16 = event_pair — "Player A vs Player B" matchup string ← KEY
  #   col17 = action_flag — player name on action/date row (19 filled)
  #   col18 = tournament name (94 filled, forward-filled)
  #   col19 = date_raw — Excel serial date (19 filled, only on action rows)
  #   col20 = time_raw / odds string (19 filled)
  #   col21 = player_short — short player display name (99 filled, every row)
  #   col22 = model_prob_v — spreadsheet-computed win prob (99 filled)
  #   col23 = implied — no-vig book prob (80 filled)
  #   col24 = edge — E(W%) minus implied (80 filled)
  colnames(raw_matches) <- c(
    ...
    "player_short", "model_prob_v", "implied", "edge"
  )

  # ── Parse into matchup pairs ──────────────────────────────────────────────
  # Rows come in pairs; header sentinel rows ("Player" in col A) separate
  # tournament blocks. We process the raw rows directly.
  ...
  parsed_pairs <- raw_matches %>%
    ...
    mutate(
      ...
      prob_c  = suppressWarnings(as.numeric(prob_c)),
      prob_d  = suppressWarnings(as.numeric(prob_d)),
      # Convert date_raw — readxl parses col 19 as POSIXct when it detects a
      # date format; fall back to Excel serial conversion for numeric columns.
      date_raw = {
      ...
      # Forward-fill all navigation columns BEFORE any row filtering
      # (action row may be dropped later if eliminated — must capture its data first)
    ...
    # ── Round-progression: determine active model prob ────────────────────
    mutate(
      # C=0 → eliminated; C=1 → advanced (use D); 0<C<1 → active (use C)
      is_eliminated = !is.na(prob_c) & prob_c == 0,
      is_advanced   = !is.na(prob_c) & prob_c == 1,
      model_prob    = case_when(
        is_eliminated            ~ NA_real_,   # skip
        is_advanced              ~ prob_d,     # next round prob
        !is.na(prob_c)           ~ prob_c,     # current round prob
        TRUE                     ~ NA_real_
      )
    ) %>%
    # Drop eliminated players and rows with no model prob
    filter(!is_eliminated, !is.na(model_prob), model_prob > 0) %>%
    # Drop rows with no odds posted
    filter(!is.na(odds), odds > 1) %>%
    # Drop rows where event_pair not yet populated (odds exist but draw not set)
    filter(!is.na(event_pair))

  # ── Reconstruct matchup pairs ─────────────────────────────────────────────
  # event_pair (col 16) = "Player A vs Player B" — grouped to get game_date
  # date_raw already forward-filled above; max() within group is now reliable
  ...
  # ── Simulator: build per-tournament draw and run Sackmann sim ─────────────
  # TA probs (prob_c / prob_d) become ta_model_prob — cross-check only.
  # Simulator output (sim_next_match_prob) is the primary model_prob.
  # Fallback to ta_model_prob when Elo unavailable for one or both players.

  strip_name <- function(x) {
    x %>%
      str_remove("^\\([^)]*\\)\\s*") %>%
      str_remove("\\s*\\([A-Z]{2,3}\\)$") %>%
      str_squish()
  }

  parsed_pairs <- parsed_pairs %>%
    rename(ta_model_prob = model_prob) %>%   # preserve TA prob for cross-check
    mutate(player_clean = strip_name(player)) %>%
    left_join(challenger_elo %>% select(player, elo),
              by = c("player_clean" = "player"))

  # Per-tournament simulation
  sim_results <- parsed_pairs %>%
    group_by(tournament) %>%
    group_modify(function(block, key) {
      draw_candidates <- block %>%
        filter(!is.na(elo)) %>%
        select(player_clean, elo) %>%
        distinct() %>%
        rename(player = player_clean)

      n_draw     <- nrow(draw_candidates)
      valid_size <- if (n_draw >= 2) 2^floor(log2(n_draw)) else 0L

      if (valid_size >= 2) {
        draw_df    <- draw_candidates %>% slice(seq_len(valid_size))
        sim_result <- tryCatch(
          sim_tournament(draw_df, sims = TENNIS_SIMS) %>%
            rename(player_clean = player),
          error = function(e) {
            message("sim_tournament failed [", key$tournament, "]: ", e$message)
            draw_df %>% rename(player_clean = player) %>%
              mutate(sim_win_prob = NA_real_, sim_reach_f = NA_real_,
                     sim_reach_sf = NA_real_, sim_next_match_prob = NA_real_)
          }
        )
        block %>% left_join(
          sim_result %>% select(player_clean, sim_win_prob, sim_reach_f,
                                sim_reach_sf, sim_next_match_prob),
          by = "player_clean"
        )
      } else {
        block %>% mutate(sim_win_prob = NA_real_, sim_reach_f = NA_real_,
                         sim_reach_sf = NA_real_, sim_next_match_prob = NA_real_)
      }
    }) %>%
    ungroup() %>%
    mutate(
      model_prob = coalesce(sim_next_match_prob, ta_model_prob),
      sim_used   = !is.na(sim_next_match_prob)
    )

  cat(sprintf("Challenger simulator: %d picks | %d sim probs | %d TA fallback\n",
              nrow(sim_results),
              sum(sim_results$sim_used, na.rm = TRUE),
              sum(!sim_results$sim_used, na.rm = TRUE)))

  # ── Bracket diagnostic — verify draw order and sim probs ─────────────────
  # Shows every player in each tournament: bracket slot, Elo, sim probs.
  # Bracket pairs are (row 1 vs row 2), (row 3 vs row 4), etc.
  cat("\n══════════════════════════════════════════════════════════════════\n")
  cat("  CHALLENGER BRACKET DIAGNOSTIC — verify draw order before betting\n")
  cat("══════════════════════════════════════════════════════════════════\n")

  sim_results %>%
    filter(!is.na(elo)) %>%              # only players that entered the simulator
    group_by(tournament) %>%
    group_walk(function(block, key) {
      cat(sprintf("\n🎾 %s\n", key$tournament))
      cat(sprintf("   %-4s  %-35s  %6s  %8s  %8s  %8s\n",
                  "Slot", "Player", "Elo", "Next", "SF", "Final"))
      cat(paste(rep("─", 76), collapse=""), "\n")

      draw_rows <- block %>%
        distinct(player_clean, elo, sim_next_match_prob,
                 sim_reach_sf, sim_reach_f) %>%
        filter(!is.na(sim_next_match_prob)) %>%
        mutate(slot = row_number())

      for (i in seq_len(nrow(draw_rows))) {
        r      <- draw_rows[i, ]
        # Mark bracket matchup pairs with a connector
        pair_marker <- if (i %% 2 == 1) "┐" else "┘"
        cat(sprintf("   %-4s  %-35s  %6.0f  %7.1f%%  %7.1f%%  %7.1f%%  %s\n",
                    paste0("[", r$slot, "]"),
                    substr(r$player_clean, 1, 35),
                    r$elo,
                    r$sim_next_match_prob * 100,
                    r$sim_reach_sf * 100,
                    r$sim_reach_f * 100,
                    pair_marker))
        # Print blank separator between pairs
        if (i %% 2 == 0 && i < nrow(draw_rows)) cat("\n")
      }
    })
  ...
  # ── Alignment: sim prob vs TA cross-check ────────────────────────────────
  # When sim_used=TRUE: compare sim_next_match_prob vs ta_model_prob
  # When sim_used=FALSE (TA fallback): flag as needing Elo
  picks_with_elo <- picks_raw %>%
    mutate(
      elo_prob  = coalesce(elo, NA_real_),    # sim already used elo; store for display
      # Alignment: compares sim_next_match_prob vs ta_model_prob
      # Divergence is EXPECTED — sim accounts for full bracket path dependency.
      # Large divergence (⚠️ Check) = our model disagrees with TA's round prob.
      # This is signal, not noise — investigate before placing.
      prob_diff = abs(model_prob - coalesce(ta_model_prob, model_prob)),
      alignment = case_when(
        !sim_used                ~ "⚠️ No Elo (TA fallback)",
        prob_diff < 0.08         ~ "✅ Elite",    # sim and TA agree closely
        prob_diff < 0.18         ~ "✅ Good",     # moderate divergence — OK
        prob_diff < 0.30         ~ "⚠️ Check",   # significant divergence — review
        TRUE                     ~ "⚠️ Large gap" # >30pp — verify draw/seeding
      )
    ) %>%
    select(-player_clean, -elo,
           -is_eliminated, -is_advanced,
           -prob_c, -prob_d, -prob_e, -prob_f,
           -col7, -col8, -col9, -col10, -col11,
           -col12, -col13, -model_prob_v_alt, -implied_alt,
           -date_raw, -time_raw, -model_prob_v, -implied, -edge,
           -event_pair, -action_flag, -sim_used) %>%
    select(sport, sport_tournament, game_date, open_time,
           home_player, away_player, bet_team,
           bet_ml, model_prob, ta_model_prob, sim_next_match_prob,
           bet_ev, raw_kelly,
           position_id, status, book,
           elo_prob, prob_diff, alignment,
...
# ── Elo name match diagnostic ────────────────────────────────────────────────
if (nrow(tennis_picks) > 0) {
  no_elo <- tennis_picks %>% filter(str_detect(alignment, "No Elo"))
  if (nrow(no_elo) > 0) {
    cat(sprintf("⚠️  %d challenger picks on TA fallback (no Elo) — add players to TEN ELO tab:\n",
                nrow(no_elo)))
    print(no_elo %>% select(sport_tournament, game_date, home_player, away_player, bet_team))
  }
}
...
if (nrow(tennis_picks) > 0) {
  ...
  print(tennis_picks %>%
          ...
          select(sport_tournament, bet_team, bet_ml,
                 model_prob, ta_model_prob, bet_ev, alignment) %>%
          head(10))
} else {
...
all_picks_df <- bind_rows(
  value_best        %>% select(sport, game_date, home_team, away_team,
                                bet_team, bet_ml, bet_ev, raw_kelly, scaled_kelly),
  spread_total_best %>% select(sport, game_date, home_team, away_team,
                                bet_team = bet_team, bet_ml = bet_odds,
                                bet_ev = bet_edge, raw_kelly, scaled_kelly),
  value_soccer      %>% select(sport, game_date, home_team, away_team,
                                bet_team, bet_ml, bet_ev, raw_kelly, scaled_kelly),
  value_mlb         %>% select(sport, game_date, home_team, away_team,
                                bet_team, bet_ml, bet_ev, raw_kelly, scaled_kelly),
  totals_mlb        %>% select(sport, game_date, home_team, away_team,
                                bet_team, bet_ml, bet_ev, raw_kelly, scaled_kelly),
  tennis_value_bets %>% select(sport, game_date,
                                home_team = home_team, away_team = away_team,
                                bet_team, bet_ml, bet_ev, raw_kelly, scaled_kelly),
  if (nrow(tennis_picks) > 0)
    tennis_picks %>% transmute(sport, game_date,
...
cap_daily_bets <- function(picks_df, bankroll = 1000, cap_pct = 0.10) {
  ...
  hard_cap <- bankroll * cap_pct          # e.g. $100 at 10%
  capped <- picks_df %>%
    arrange(desc(bet_ev_pct)) %>%         # best EV bets get funded first
    mutate(
      risk_dollar = scaled_kelly * bankroll,
      cum_risk    = cumsum(risk_dollar),
      # Scale down the last bet that crosses the cap rather than dropping it
      final_risk  = case_when(
        cum_risk - risk_dollar >= hard_cap ~ 0,           # fully over cap → drop
        cum_risk > hard_cap ~ hard_cap - (cum_risk - risk_dollar), # partial
        TRUE ~ risk_dollar                                # fully under cap → keep
      )
    ) %>%
    filter(final_risk > 0) %>%
    select(-cum_risk)

  cat(sprintf("CAPPED: %.1f%% risk ($%.0f / $%.0f bankroll), %d bets\n",
              sum(capped$final_risk) / bankroll * 100,
              sum(capped$final_risk), bankroll, nrow(capped)))
  return(capped)
}
...
write_csv(final_picks, daily_picks_path)
cat(sprintf("✅ Daily picks written: %d bets → %s\n",
            nrow(final_picks), daily_picks_path))
...
# TOA /scores returns completed matches for any sport_key.
# We hit each tennis key we actually fetched odds for.
fetch_tennis_results <- function(target_date = Sys.Date() - 1) {
  # Get all tennis keys that were in our odds fetch
  ...
  map_dfr(tennis_keys_live, function(sk) {
    tryCatch({
      raw <- toa_sports_scores(
        sport_key        = sk,
        days_from        = 1,      # look back 1 day
        date_format      = "iso"
      )
      if (is.null(raw) || nrow(raw) == 0) return(tibble())

      # TOA scores columns vary by version — detect score cols defensively
      score_cols <- names(raw)[grepl("score", names(raw), ignore.case = TRUE)]
      home_col   <- score_cols[grepl("home", score_cols, ignore.case = TRUE)][1]
      away_col   <- score_cols[grepl("away", score_cols, ignore.case = TRUE)][1]

      if (is.na(home_col) || is.na(away_col)) {
        message("Tennis settler: score columns not found for ", sk,
                " — columns: ", paste(names(raw), collapse=", "))
        return(tibble())
      }

      raw %>%
        filter(!is.na(.data[[home_col]]) | !is.na(.data[[away_col]])) %>%
        mutate(
          sport_key  = sk,
          game_date  = as.Date(substr(commence_time, 1, 10)),
          home_score = as.numeric(.data[[home_col]]),
          away_score = as.numeric(.data[[away_col]]),
          completed  = completed,
          winner     = case_when(
            completed & home_score > away_score ~ home_team,
            completed & away_score > home_score ~ away_team,
            TRUE ~ NA_character_
          )
        ) %>%
        filter(game_date == target_date, completed == TRUE, !is.na(winner)) %>%
        select(sport_key, game_date, home_team, away_team, winner)
    }, error = function(e) {
...
fetch_challenger_results <- function(target_date = Sys.Date() - 1) {
  url <- paste0(
    "http://site.api.espn.com/apis/site/v2/sports/tennis/ten/scoreboard?dates=",
    format(target_date, "%Y%m%d")
  )
  raw <- tryCatch(fromJSON(url), error = function(e) {
    message("ESPN Challenger fetch failed: ", e$message)
    return(NULL)
  })
  ...
  map_dfr(seq_len(nrow(raw$events)), function(i) {
    tryCatch({
      comp      <- raw$events$competitions[[i]]
      stype     <- comp$status$type
      completed <- isTRUE(stype$completed) || isTRUE(stype$name == "STATUS_FINAL")
      if (!completed) return(tibble())

      competitors <- comp$competitors[[1]]
      # ESPN tennis: winner flagged by winner == TRUE
      ...
      tibble(
        game_date    = as.Date(substr(raw$events$date[i], 1, 10)),
        winner_raw   = winner_row$athlete$displayName %||% winner_row$team$displayName,
        loser_raw    = loser_row$athlete$displayName  %||% loser_row$team$displayName,
        winner_clean = clean_tennis_name(winner_raw),
        loser_clean  = clean_tennis_name(loser_raw)
      )
    }, error = function(e) {
      message("ESPN Challenger parse error event ", i, ": ", e$message)
      tibble()
    })
  })
}
