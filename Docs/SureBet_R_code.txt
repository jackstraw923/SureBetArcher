...
calc_tennis_value <- function(best_odds_df, elo_df, tour_label) {
  ...
  best_odds_df %>%
    ...
    mutate(
      ...
      # +25 Elo points HFA for indoor/outdoor tournaments (standard adjustment)
      elo_diff        = home_elo - away_elo + 25,
      model_home_prob = 1 / (1 + 10^(-elo_diff / 400)),
...
# ============================================================
# TENNIS CHALLENGER PIPELINE — Spreadsheet + Elo Verification
# ============================================================
# Data source: local Excel file (SUREBET_DIR/2025TENRef3.xlsx)
# Unlike ATP/WTA, Challenger odds come from the spreadsheet model
# (Sackmann ev_calc / model_prob), not from TOA live odds.
# ── Step 2 Elo block: hardcoded per tournament — update ratings
#    each week as the draw changes. Replace with live scrape when
#    a Challenger Elo source is identified.

challenger_xlsx <- file.path(SUREBET_DIR, "2025TENRef3.xlsx")

tennis_picks <- if (file.exists(challenger_xlsx)) {

  # ── 1. Load + process ──────────────────────────────────────
  raw_data <- read_xlsx(challenger_xlsx, sheet = "Matches")

  picks_raw <- raw_data %>%
    rename(
      player           = Player,
      odds_espnbet     = Odds,
      model_prob       = Column113,   # Sackmann win probability
      ev_calc          = Column112,
      action_flag      = Column12,
      sport_tournament = Sport,
      game_date_raw    = Date,
      game_time        = Time,
      event_name       = Event
    ) %>%
    filter(!is.na(model_prob), model_prob > 0, !is.na(odds_espnbet)) %>%
    mutate(
      game_date    = as.Date(game_date_raw),
      sport        = "TENNIS-CHALLENGER",
      home_player  = str_extract(event_name, "^[^v]+(?= vs)"),
      away_player  = str_extract(event_name, "(?<=vs )[^$]+"),
      bet_team     = player,
      bet_ml       = odds_espnbet,
      bet_ev       = ev_calc,
      # Raw Kelly — will be rescaled by global sf_combined below
    ...
    filter(bet_ev > EV_GATE) %>%
    select(sport, sport_tournament, game_date, open_time,
           home_player, away_player, bet_team,
           bet_ml, model_prob, bet_ev, raw_kelly,
           position_id, status, book, action_flag,
  ...
  # ── 2. Elo verification — UPDATE RATINGS EACH TOURNAMENT ───
  # Replace hardcoded values with live lookup when source available.
  picks_raw %>%
    mutate(
      elo_home = case_when(
        str_detect(home_player, "Arthur Cazaux")                    ~ 1825,
        str_detect(home_player, "Eero Vasa")                        ~ 1301,
        str_detect(home_player, "Marco Cecchinato|Marco Trungelliti") ~ 1450,
        str_detect(home_player, "Clement Tabur")                    ~ 1520,
        TRUE                                                         ~ 1600
      ),
      elo_away = case_when(
        str_detect(away_player, "Jacob Fearnley|Billy Harris")      ~ 1750,
        str_detect(away_player, "Filip|Jianu")                      ~ 1650,
        str_detect(away_player, "Nick Hardt")                       ~ 1450,
        str_detect(away_player, "Filippo Romano")                   ~ 1480,
        TRUE                                                         ~ 1550
      ),
      elo_diff      = elo_home - elo_away,
      elo_prob_home = 1 / (1 + 10^(-elo_diff / 400)),
      elo_prob      = if_else(bet_team == home_player,
                              elo_prob_home, 1 - elo_prob_home),
      prob_diff     = abs(model_prob - elo_prob),
      alignment     = case_when(
    ...
    select(-elo_home, -elo_away, -elo_diff, -elo_prob_home)
...
if (nrow(tennis_picks) > 0) {
  ...
  elite_bets <- tennis_picks %>%
    filter(bet_ev > 0.10, alignment %in% c("✅ Elite", "✅ Good"))
  if (nrow(elite_bets) > 0) {
    elite_path <- file.path("logs",
                            paste0("elite_challenger_bets_", Sys.Date(), ".csv"))
    write_csv(elite_bets, elite_path)
    cat(sprintf("Elite challenger bets written: %d → %s\n",
                nrow(elite_bets), elite_path))
  }
}
