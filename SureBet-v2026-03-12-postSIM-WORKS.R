...
tennis_picks <- if (file.exists(challenger_xlsx)) {
  ...
  colnames(raw_matches) <- c(
    ...
    paste0("col", 7:11),
    "col12", "col13", "col14", "col15", "event_pair",
    "action_flag", "tournament", "date_raw", "time_raw",
    "event_name", "model_prob_v", "implied", "edge"
  )
  ...
  parsed_pairs <- raw_matches %>%
    ...
    mutate(
      ...
      date_raw = as.Date(as.numeric(date_raw),
                         origin = "1899-12-30"),   # Excel serial date
      # Forward-fill tournament name (only populated on first row of block)
  ...
  # event_name = "Player A vs Player B" — use to pair rows
  # Forward-fill date from the "action" row (row with a date) within each pair
  parsed_pairs <- parsed_pairs %>%
    group_by(tournament, event_name) %>%
    mutate(
      game_date   = as.Date(max(date_raw, na.rm = TRUE)),
      home_player = str_trim(str_extract(event_name, "^.+?(?= vs )")),
      away_player = str_trim(str_extract(event_name, "(?<= vs ).+$"))
    ) %>%
    ungroup() %>%
    # game_date may be NA if no date in that pair yet (odds not posted)
...
fetch_tennis_results <- function(target_date = Sys.Date() - 1) {
  ...
  map_dfr(tennis_keys_live, function(sk) {
    tryCatch({
      ...
      raw %>%
        filter(!is.na(scores_home_score) | !is.na(scores_away_score)) %>%
        mutate(
          sport_key  = sk,
          game_date  = as.Date(substr(commence_time, 1, 10)),
          home_team  = home_team,
          away_team  = away_team,
          home_score = as.numeric(scores_home_score),
          away_score = as.numeric(scores_away_score),
          completed  = completed,
          winner     = case_when(
